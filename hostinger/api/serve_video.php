<?php
/**
 * API para servir vídeos com autenticação e suporte a streaming
 * Baseada em serve_content_file.php, mas otimizada para streaming de vídeo
 * Suporta HTTP Range requests para reprodução eficiente de vídeos
 */
...
// Desabilitar exibição de erros para não quebrar o streaming
error_reporting(E_ALL);
ini_set('display_errors', 0);
ini_set('log_errors', 1);

// Headers CORS e de resposta para streaming
...
try {
    ...
    error_log("Erro ao carregar includes em serve_video.php: " . $e->getMessage());
    http_response_code(500);
    die('Erro ao inicializar servidor.');
} catch (Error $e) {
    error_log("Erro fatal ao carregar includes em serve_video.php: " . $e->getMessage());
    http_response_code(500);
...
if (!$conn || !($conn instanceof mysqli)) {
    error_log("Erro: Conexão com banco de dados não estabelecida em serve_video.php");
    http_response_code(500);
...
try {
    ...
    if ($check_files_table && $check_files_table->num_rows > 0) {
        if ($file_id > 0) {
            // Buscar por ID do arquivo - APENAS VÍDEOS
            $stmt = $conn->prepare("SELECT cf.*, mc.id as content_id FROM sf_content_files cf 
                                    ...
                                    WHERE cf.id = ? AND cf.mime_type LIKE 'video/%'");
            $stmt->bind_param("i", $file_id);
        } else {
            // Buscar por content_id (pegar o primeiro arquivo de vídeo)
            $stmt = $conn->prepare("SELECT cf.*, mc.id as content_id FROM sf_content_files cf 
                                    INNER JOIN sf_member_content mc ON cf.content_id = mc.id 
                                    WHERE cf.content_id = ? AND cf.mime_type LIKE 'video/%'
        ...
        if ($result->num_rows > 0) {
            ...
            $mime_type = $file_data['mime_type'] ?? 'video/mp4';
    ...
    if (!$file_path && $content_id > 0) {
        $stmt = $conn->prepare("SELECT file_path, file_name, mime_type FROM sf_member_content WHERE id = ? AND (mime_type LIKE 'video/%' OR file_path LIKE '%.mp4' OR file_path LIKE '%.mov' OR file_path LIKE '%.avi' OR file_path LIKE '%.webm')");
        $stmt->bind_param("i", $content_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($result->num_rows > 0) {
            ...
            if (!empty($content_data['file_path'])) {
                ...
                $mime_type = $content_data['mime_type'] ?? 'video/mp4';
            }
        }
        $stmt->close();
    }
    ...
    if (!$file_path) {
        error_log("serve_video.php: file_path não encontrado para file_id={$file_id}, content_id={$content_id}");
        http_response_code(404);
        die('Arquivo de vídeo não encontrado no banco de dados.');
    }
    
    error_log("serve_video.php: file_path encontrado: {$file_path}, file_name: {$file_name}, content_id: {$content_id}");
    ...
    if (!$found) {
        error_log("serve_video.php: Arquivo não encontrado. file_path: {$file_path}");
        http_response_code(404);
        die('Arquivo de vídeo não encontrado no servidor.');
    }
    ...
    $fileSize = filesize($physical_path);
    ...
    // ✅ HEADERS PARA STREAMING DE VÍDEO (não download)
    header('Content-Type: ' . $mime_type);
    header('Accept-Ranges: bytes');
    header('Content-Length: ' . $fileSize);
    header('Cache-Control: public, max-age=3600');
    // ✅ NÃO usar Content-Disposition: attachment (isso força download)
    
    // ✅ SUPORTE A HTTP RANGE REQUESTS (essencial para streaming)
    ...
    if ($range) {
        ...
        if (preg_match('/bytes=(\d+)-(\d*)/', $range, $matches)) {
            ...
            // Limpar qualquer output antes de enviar
            if (ob_get_level()) {
                ob_end_clean();
            }
            
            // Abrir arquivo e enviar apenas a parte solicitada
            $fp = fopen($physical_path, 'rb');
            fseek($fp, $start);
    ...
    // Se não há Range request, enviar arquivo completo
    // Limpar qualquer output antes de enviar
    if (ob_get_level()) {
        ob_end_clean();
    }
    
    // Para arquivos grandes, usar readfile() é mais eficiente
    readfile($physical_path);
    exit;
    
} catch (Exception $e) {
    error_log("Erro em serve_video.php: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
    http_response_code(500);
    die('Erro ao servir vídeo.');
} catch (Error $e) {
    error_log("Erro fatal em serve_video.php: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
    http_response_code(500);
    die('Erro fatal ao servir vídeo.');
}
?>
