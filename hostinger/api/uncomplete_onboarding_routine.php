<?php
// api/uncomplete_onboarding_routine.php
...
try {
    ...
    if ($stmt_delete->affected_rows === 0) {
        $stmt_delete->close();
        $conn->rollback();
        echo json_encode(['success' => false, 'message' => 'Nenhuma atividade de onboarding correspondente encontrada para desfazer.']);
        exit();
    }
    $stmt_delete->close();
    ...
    $conn->rollback();
    error_log("Erro ao desfazer onboarding para user {$user_id}: " . $e->getMessage());
