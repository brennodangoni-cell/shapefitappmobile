...
try {
    ...
    if (is_array($user_id) || is_object($user_id)) {
        ...
        // Primeiro, tentar chave 'id'
        if (isset($user_id_array['id'])) {
            $extracted_id = $user_id_array['id'];
        }
        // Se não, tentar chave 'user_id'
        elseif (isset($user_id_array['user_id'])) {
            $extracted_id = $user_id_array['user_id'];
        }
        // Se não, tentar primeiro elemento do array
        elseif (isset($user_id_array[0])) {
            $extracted_id = $user_id_array[0];
            // Se o primeiro elemento também for array/objeto, tentar extrair id dele
            if (is_array($extracted_id) || is_object($extracted_id)) {
                $extracted_id_array = (array)$extracted_id;
                $extracted_id = isset($extracted_id_array['id']) ? $extracted_id_array['id'] : null;
            }
        }
        
        logDeleteAccount("user_id extraído do objeto", [
            'original_type' => is_object($original_user_id) ? 'object' : 'array',
            'original_keys' => array_keys($user_id_array),
